...
public:
    ...
    Download(std::mutex& mtx, lt::add_torrent_params& atp, bool keep);
    ~Download();

    static std::shared_ptr<Download>
    get_download(char* metadata, size_t metadatalen, std::string save_path, bool keep);

    ssize_t read(int file, int64_t off, char* buf, size_t buflen, DataProgressCb progress_cb);
    ssize_t read(int file, int64_t off, char* buf, size_t buflen) {
    ...
    std::vector<std::pair<std::string, uint64_t>> get_files();
    ...
    std::shared_ptr<std::vector<char>> get_metadata(MetadataProgressCb progress_cb);
    std::shared_ptr<std::vector<char>> get_metadata() { return get_metadata(nullptr); }

    std::pair<int, uint64_t> get_file(std::string path);
    std::string get_name();
...
private:
    static std::shared_ptr<Download> get_download(lt::add_torrent_params& atp, bool keep);

    void download_metadata(MetadataProgressCb cb);
    void download_metadata() { download_metadata(nullptr); }

    void download(lt::peer_request part, DataProgressCb cb);
    void download(lt::peer_request part) { download(part, nullptr); }
